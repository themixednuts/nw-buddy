...
-- Sets RLS Policy on SELECT for GEARSETS to PUBLIC. So anyone can select a GEARSET
...
CREATE POLICY "Users can insert their own gearset." ON gearsets FOR
...
CREATE POLICY "Users can update own gearset." ON gearsets FOR
...
CREATE POLICY "Users can delete own gearset." ON gearsets FOR DELETE USING (
  (
    SELECT auth.uid ()
  ) = gearsets.user_id
);
...
CREATE OR REPLACE FUNCTION handle_gearset_updated_at() RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
...
-- Create the trigger
CREATE TRIGGER handle_gearset_updated_at BEFORE
UPDATE ON gearsets FOR EACH ROW EXECUTE FUNCTION handle_gearset_updated_at();