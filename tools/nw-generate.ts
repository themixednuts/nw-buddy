...
program
  ...
  .action(async (data) => {
    ...
    for (const file of files) {
      ...
      const data = fs.readFileSync(file, { encoding: "utf8" }).replaceAll(
        /\s+/g,
        " ",
      );
      fs.writeFileSync(
        tsFile,
        `export const ${constName} = ${JSON.stringify(data)}`,
      );
    }
...
program
  ...
  .action(async ({ config }: { config: string }) => {
    ...
    const envFile = path.join(
      environment.appsDir("web", "environments", "env.ts"),
    );
    const envFileGenerated = path.join(
      environment.appsDir("web", "environments", "env.generated.ts"),
    );
    ...
    const env = {
      ...
      cdnUrl: CDN_URL,
      deployUrl:
        ngConfig.projects["nw-buddy"].architect.build.configurations[config]
          .baseHref || "/",
...
program
  ...
  .action(() => {
    ...
    const versionFile = path.join(
      environment.distDir("web", "browser", "version"),
    );
