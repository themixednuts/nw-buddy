...
@Component({
  ...
  animations: [
    ...
    trigger("enteranimation", [
      transition(":enter", [
        style({ opacity: 0 }),
        animate("1s 0.3s ease-out", style({ opacity: 1 })),
      ]),
    ]),
    trigger("appear", [
      transition(":enter", [
...
export class AppComponent {
  ...
  public get isWeb() {
    return this.platform.env.environment === "WEB" ||
      this.platform.env.environment === "DEV";
  }
  ...
  protected langSelection = selectSignal(injectRouteParam("locale"), (it) => {
    return LANG_OPTIONS.find((lang) => lang.value === it) ||
      LANG_OPTIONS.find((lang) => lang.isDefault);
  });
  protected langLabel = computed(() =>
    this.langSelection().value.split("-")[0].toUpperCase()
  );
